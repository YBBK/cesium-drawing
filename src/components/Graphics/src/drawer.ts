...
import { DrawableGraphic, PointGraphic, LineGraphic, PolygonGraphic, RectangleGraphic, CircleGraphic } from './drawableGraphic';
...
class Drawer {
    ...
    private enableDrawingListeners() {
        ...
        this._handler.setInputAction(this.dbClickHandler.bind(this), Cesium.ScreenSpaceEventType.LEFT_DOUBLE_CLICK);
        this._handler.setInputAction(this.rightClickHandler.bind(this), Cesium.ScreenSpaceEventType.RIGHT_CLICK);
        this._handler.setInputAction(this.mouseMoveHandler.bind(this), Cesium.ScreenSpaceEventType.MOUSE_MOVE);
    }

    // 启用编辑相关的事件监听器
    private enableEditingListeners() {
        this._handler.setInputAction(this.editClickHandler.bind(this), Cesium.ScreenSpaceEventType.LEFT_CLICK);
        this._handler.setInputAction(this.editMoveHandler.bind(this), Cesium.ScreenSpaceEventType.MOUSE_MOVE);
        this._handler.setInputAction(this.editDownHandler.bind(this), Cesium.ScreenSpaceEventType.LEFT_DOWN);
    ...
    private enableNormalListeners() {
        this._handler.setInputAction(this.pickClickHandler.bind(this), Cesium.ScreenSpaceEventType.LEFT_CLICK);
    }
    ...
    private editMoveHandler(e: Cesium.ScreenSpaceEventHandler.MotionEvent) {
        ...
        if (!this._isDraging || (!this._selectedNode && !this._selectedGraphic) || !this._startMousePosition) {
            return;
        }
        ...
        if (this._selectedNode) {
            ...
            const diff = Cesium.Cartesian3.subtract(endPos, this._startMousePosition, new Cesium.Cartesian3());
    ...
    private startDrawing(pos: Cesium.Cartesian3) {
        ...
        switch (this._type) {
            case GraphicType.Point:
                this._currentGraphic = new PointGraphic(this._viewer, id, name, [pos], this._options, this.notifyGraphicUpdated.bind(this));
                this._currentGraphic.show();
                this.finishDrawing();
                return;
            case GraphicType.PolyLine:
                this._currentGraphic = new LineGraphic(this._viewer, id, name, [pos], this._options, this.notifyGraphicUpdated.bind(this));
                break;
            case GraphicType.Polygon:
                this._currentGraphic = new PolygonGraphic(this._viewer, id, name, [pos], this._options, this.notifyGraphicUpdated.bind(this));
                break;
            case GraphicType.Circle:
                this._currentGraphic = new CircleGraphic(this._viewer, id, name, [pos], this._options, this.notifyGraphicUpdated.bind(this));
                break;
            case GraphicType.Rectangle:
                this._currentGraphic = new RectangleGraphic(this._viewer, id, name, [pos], this._options, this.notifyGraphicUpdated.bind(this));
        }
        if (this._currentGraphic) {
            this._currentGraphic.show();
        }
    ...
    private createGraphicFromFeature(feature: GeoJSON.Feature, isNotify: boolean = false) {
        ...
        switch (feature.geometry.type) {
            case 'Point':
            case 'MultiPoint':
                graphic = new PointGraphic(this._viewer, id, name, coordinates, _options, this.notifyGraphicUpdated.bind(this));
                break;
            case 'LineString':
            case 'MultiLineString':
                graphic = new LineGraphic(this._viewer, id, name, coordinates, _options, this.notifyGraphicUpdated.bind(this));
                break;
            case 'Polygon':
            case 'MultiPolygon':
    ...
    private getCoordinatesFromGeometry(geometry: GeoJSON.Geometry): Cesium.Cartesian3[] {
        ...
        return coordinates.map((coord) => Cesium.Cartesian3.fromDegrees(coord[0], coord[1], coord[2] || 0));
    }
