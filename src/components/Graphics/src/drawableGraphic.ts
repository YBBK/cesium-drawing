...
abstract class DrawableGraphic extends BaseGraphic {
  ...
  protected _updateCallback: (arg: BaseGraphic) => void
  ...
  protected showNodes() {
    this._nodes.forEach((node, index) => {
      if (node.point) {
        const size =
          this._selectedNodeIndex == index
            ? (this._options.pixelSize || 5) * 1.5
            : this._options.pixelSize
  ...
  protected createNodes() {
    this._positions.forEach((position, index) => {
      ...
      if (this.isAllowControlPoint()) {
        ...
        const last =
          this.type() == GraphicType.Polygon ? this._positions.length : this._positions.length - 1
        if (index < last) {
          const nextPosition =
            index < this._positions.length - 1 ? this._positions[index + 1] : this._positions[0]
    ...
    const entity = new Cesium.Entity({
      polyline: {
        positions: new Cesium.CallbackProperty(
          () => [...this._positions, ...this._dynamicPositions],
          false
        ),
...
class RectangleGraphic extends DrawableGraphic {
  ...
  protected generateEdgePoints(
    leftTopPoint: Cesium.Cartesian3,
    rightBottomPoint: Cesium.Cartesian3
  ): Cesium.Cartesian3[] {
    ...
    // 计算右上角和左下角的坐标
    const rightTopCartographic = new Cesium.Cartographic(
      rightBottomCartographic.longitude,
      leftTopCartographic.latitude
    )
    const leftBottomCartographic = new Cesium.Cartographic(
      leftTopCartographic.longitude,
      rightBottomCartographic.latitude
    )
    const rightTop = Cesium.Cartesian3.fromRadians(
      rightTopCartographic.longitude,
      rightTopCartographic.latitude
    )
    const leftBottom = Cesium.Cartesian3.fromRadians(
      leftBottomCartographic.longitude,
      leftBottomCartographic.latitude
    )
