            ...
            <Panel>
                ...
                <template #extra>
                    <ButtonGroup direction="h">
                        <div class="button" @click="handleImport()">
                            <Icon icon="import" />
                            导入
                        </div>
                ...
                <ul class="layer-list">
                    <li v-for="(shapes, type) in store.shapes" :key="type">
                        <div class="item">
                            ...
                            <div class="action">
                                <Icon icon="delete" @click="handleClear(type)" />
                            </div>
                        </div>
                        <ul v-if="isGroupOpen(type)">
                            <li v-for="(shape, idx) in shapes" :key="idx" class="item leaf">
                                ...
                                <div @dblclick="handleEditName(shape.id)" v-if="!isEditing(shape.id)" title="双击修改">{{ shape.properties.name }}</div>
                                <div v-else>
                                    <input type="text" :id="'input_ele_' + shape.id" :value="shape.properties.name" @keydown.enter="handleEditNameCompleted($event, shape.id)" />
                                </div>
                                <div class="action">
                                    <Icon icon="edit" @click="handleEditName(shape.id)" v-if="!isEditing(shape.id)" />
                                    <Icon icon="check" @click="handleEditNameSubmit(shape.id)" v-else />
                                    <Icon icon="aim" @click="handleFlyTo(shape.id)" />
                                    <Icon icon="delete" @click="handleDelete(shape.id)" />
                                </div>
                            </li>
                        </ul>
                    </li>
