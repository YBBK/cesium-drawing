<template>
  <MapViewer ref="mapViewer" class="map_viewer">
    <template #action>
      <IconButton icon="import" @click="handleImport()"></IconButton>
    </template>
    <template #sideLayer>
      <Panel>
        ...
        <ul class="layer-list">
          <li v-for="(shapes, type) in store.shapes" :key="type">
            <div class="item">
              <Icon
                icon="caret"
                :class="{ open: isGroupOpen(type) }"
                @click="handleToggleGroup(type)"
              />
            ...
            <ul v-if="isGroupOpen(type)">
              <li v-for="(shape, idx) in shapes" :key="idx" class="item leaf">
                ...
                <div
                  @dblclick="handleEditName(shape.id)"
                ...
                <div class="action">
                  <IconButton
                    :border="0"
                    icon="edit"
                    @click="handleEditName(shape.id)"
                    v-if="!isEditing(shape.id)"
                  />
                  <IconButton
                    :border="0"
                    icon="check"
                    @click="handleEditNameSubmit(shape.id)"
                    v-else
                  />
